...
- uses the built-in Edge runtime `SUPABASE_SERVICE_ROLE_KEY` for privileged queue updates.
...
- **`process-report-step` returns `401 Unauthorized`**  
  Verify the caller is authenticated and the function is deployed without `--no-verify-jwt`. The function now expects a valid user JWT and rejects anonymous requests.
